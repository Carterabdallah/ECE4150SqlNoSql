...
from env import AWS_ACCESS_KEY, AWS_SECRET_ACCESS_KEY, AWS_REGION, PHOTOGALLERY_S3_BUCKET_NAME, RDS_DB_HOSTNAME, RDS_DB_USERNAME, RDS_DB_PASSWORD, RDS_DB_NAME, HASH_SERIALIZER, CURRENT_URL, EMAIL_SOURCE
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        conn=get_database_connection()
        cursor = conn.cursor ()
        statement1 = f'''SELECT userID, password, email, validated FROM photogallerydb.User WHERE email="{email}";'''
...
def signup_page():
    ...
    get:
        description: Endpoint to return sign Up page.
    post:
        description: Adds user to database if current credentials don't already exist
    """
    if request.method == 'POST':
        userID = uuid.uuid4()
        email = request.form['email']
        ...
        conn=get_database_connection()
        cursor = conn.cursor ()
        statement1 = f'''SELECT userID, email FROM photogallerydb.User WHERE userID="{userID}" or email="{email}";'''
        cursor.execute(statement1)
        results = cursor.fetchall()
        if len(results) != 0:
            ...
            contentofEmail = "Please Click the link below to verify your acount \n" + CURRENT_URL+"/confirm/" + token
            send_email(email, contentofEmail)
            statement2 = f'''INSERT INTO photogallerydb.User (userId, email, firstName, lastName, password, validated) VALUES ("{userID}", "{email}", "{firstName}", "{lastName}", "{hashedPass.decode('utf-8')}", "FALSE");'''    
            result2 = cursor.execute(statement2)
            conn.commit()
            conn.close()
            return redirect('/')
    else:
...
def confirm_user(ID):
    """
    get:
        description: Endpoint to confirm user's identity
        returns: Returns user to homepage
    """
    try:
        serializer = URLSafeTimedSerializer(HASH_SERIALIZER)
        ...
        conn=get_database_connection()
        cursor = conn.cursor ()
        statement1 = f'''UPDATE photogallerydb.User SET validated="1" WHERE email="{email}";'''
        result1 = cursor.execute(statement1)
        conn.commit()
        conn.close()
        return redirect('/')
    except Exception as e:
...
def delete_photos(albumID):
    """ Delete Album photos page route.

    get:
        description: Endpoint to delete all photos from an album.
        responses: Returns the user to the new empty album page
    """
    conn=get_database_connection()
    cursor = conn.cursor ()
    # Photos
...
def cancel_user():
    """ Cancels a User and therefore Deletes All Albums On Their Account
...
def delete_photo(albumID, photoID):
    conn=get_database_connection()
    cursor = conn.cursor ()
    # Delete the Photo we are looking at
